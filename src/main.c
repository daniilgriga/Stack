...
struct stack_str
{
    ON_CNR_PRTCT (int canary_stack_1;)
    ...
    ON_CNR_PRTCT (int canary_stack_2;)
};
...
int stack_dump (struct stack_str* stack, const char* file, int line, const char* func)
{
    ...
    ON_CNR_PRTCT ( unsigned int buf_temp_1 = 0;
                   memcpy(&buf_temp_1, &stack->data[-1], sizeof(CANARY_BUFFER_VALUE));
                   printf("\ncanary_3 in buffer = "BLUE_TEXT("0x%x")"\n\n", buf_temp_1);

                   unsigned int buf_temp_2 = 0;
                   memcpy(&buf_temp_2, &stack->data[stack->capacity], sizeof(CANARY_BUFFER_VALUE));
...
int stack_error (struct stack_str* stack)
{
    ...
    ON_CNR_PRTCT ( if (stack->canary_stack_1 != CANARY_VALUE || stack->canary_stack_2 != CANARY_VALUE)
